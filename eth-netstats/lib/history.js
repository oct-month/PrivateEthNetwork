...
var filePropagationLog = fs.createWriteStream("propagation-data.txt", {flags:'a'});
var fileBlockDetailsLog = fs.createWriteStream("block-data.txt", {flags:'a'});
var lastLogTime = 0;
...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
		...
		else
		{
			...
			//console.log('Couldnt find block with this height:', block.number);

			// Getting previous max block
			// var prevBlock = this.prevMaxBlock(block.number);
			var prevBlock = this.search(block.number - 1);
			...
			{
				block.time = Math.max(block.arrived - prevBlock.block.arrived, 0);
				console.log('block.time using arrived:', block.time);
				block.time = 0;
				console.log('reset block.time arrived to 0');

				if(block.number < this.bestBlock().height) {
					block.time = Math.max((block.timestamp - prevBlock.block.timestamp) * 1000, 0);
					console.log('block.time using timestamp:', block.time);
				}
			}
			else
			{
			...
			console.log('Set block.time to:', block.time);
			...
			if( 
				...
				(this._items.length > 0 && block.number > this.worstBlockNumber()) || 
...
{
	fileBlockDetailsLog.write(JSON.stringify({n: block.block.number, p: block.propagTimes }) + "\n");
...
{
	console.log('getBlockPropagation...')
	var propagation = [];
	...
	_.forEach(this._items, function (n, key)
	{
		_.forEach(n.propagTimes, function (p, i)
		{
			...
			if (prop > 0) // propagation time should not be 0
	...
	try {
		// write block propagation data to log file
		var dnow = new Date();
		var timeSinceLastLog = Math.abs(dnow.getTime() - lastLogTime) / 1000;
		console.log('checking timeSinceLastLog:', timeSinceLastLog)
		if (timeSinceLastLog > 3600*4) {
			console.log('writing propagation data to file...')
			// write to log only once per hour
			var propLogLine = {
				'timestamp': dnow.toString(),
				'propagationArray': propagation,
				'avgPropagation': avgPropagation
			};
			lastLogTime = dnow.getTime();
			filePropagationLog.write(JSON.stringify(propLogLine) + "\n");
		}

	} catch(e) {
		console.log('error logging propagation:', e)
	}
...
{
	//console.log('History.prototype.getBlockTimes');
...
{
	//console.log('History.prototype.getAvgBlocktime.');
	var blockNums = _( this._items )
		.sortByOrder( 'height', false )
		.filter(function (item) {
	...
	//console.log('blockTimes:', blockTimes);
	//console.log('sum of blockTimes:', _.sum(blockTimes));
	//console.log('blockTimes.length:', blockTimes.length);
	//console.log('avgBlockBlocktime:', _.sum(blockTimes) / (blockTimes.length === 0 ? 1 : blockTimes.length));
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		...
		//console.log('History.prototype.getCharts.');
		//console.log('pluck chartHistory blocktime:', _.pluck( chartHistory, 'blocktime'));

...
let rawPropagationLog = fs.createWriteStream('raw-prop.txt', {flags: 'a'});
...
Collection.prototype.addBlock = function(id, stats, callback)
{
	...
	else
	{
		...
		else
		{
			...
			rawPropagationLog.write(JSON.stringify({"ip": node.info.ip, "received": stats.received, "number": block.block.number }));
...
{
	//console.log('Collection.prototype.addHistory');
